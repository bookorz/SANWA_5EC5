    ...
    public sealed class Item
    {
        ...
        private Item(IList<Item> items)
        {
            ...
            _rawData = new Lazy<byte[]>(()
                => new byte[]{
                    (byte)SecsFormat.List | 1,
                    unchecked((byte)(Unsafe.As<IList<Item>>(_values).Count))
                });
        }

        internal static Item B(object p)
        {
        ...
        public IList<Item> Items => Format != SecsFormat.List
            ? throw new InvalidOperationException("The item is not a list")
            : Unsafe.As<IList<Item>>(_values);

        internal void SetString(object obj)
        {
            throw new NotImplementedException();
        }
        ...
        public void SetValue<T>(T value)
        {
            ...
            //if (_values is T[] arr) arr[0] = value;
            if (_values is T[] arr)
            {
                arr[0] = value;
            }
        ...
        //public SetString(string value) => Format != SecsFormat.ASCII && Format != SecsFormat.JIS8
        //    ? throw new InvalidOperationException("The type is incompatible")
        //    : (_values = value);
        ...
        public void SetValues<T>(T[] value) where T : unmanaged
        {
            if (Format == SecsFormat.List)
                throw new InvalidOperationException("The item is list");
        ...
        public bool IsMatch(Item target)
        {
            ...
            switch (target.Format)
            {
                case SecsFormat.List:
                    return IsMatch(
                        ...
                        //Unsafe.As<IList<Item>>(_values),
                        //Unsafe.As<IList<Item>>(target._values));
            ...
            //bool IsMatch(IList<Item> a, IList<Item> b)
            {
        ...
        public override string ToString()
        {
            ...
            switch (Format)
            {
                case SecsFormat.List:
                    ...
                    sb.Append(Unsafe.As<IList<Item>>(_values).Count).Append("]: ...");
                    break;
                case SecsFormat.ASCII:
                case SecsFormat.JIS8:
