...
using static Rorze.SECS.Driver.Item;
...
namespace Rorze.SECS.Driver
{
    public static class SmlExtension
    {
        ...
        public static SecsMessage ToSecsMessage(this TextReader sr)
        {
            ...
            int i = line.IndexOf(':');
            var name = line.Substring(0, line.Length);
            var s = 0;
            var f = 0;
            ...
            if (i != -1)
            {
                ...
                i = line.IndexOf("'S", i + 1, StringComparison.Ordinal) + 2;
                int j = line.IndexOf('F', i);
                s = byte.Parse(line.Substring(i, j - i));

                i = line.IndexOf('\'', j);
                f = byte.Parse(line.Substring(j + 1, i - (j + 1)));
            }
            else
            {
                //20200818 Pingchung ToSML和ToSecsMessage可以互相轉換
                if (line.Contains("\' W"))
                {
                    i = line.LastIndexOf('W');
                }
                else
                {
                    i = line.LastIndexOf('\'');
                ...
                name = line.Substring(i + 1, line.Length - i - 1);
                name = name.Trim();
                //name = line.Substring(line.LastIndexOf('\''), line.Length).Trim();
                i = line.IndexOf("'S", i + 1, StringComparison.Ordinal) + 2;
                int j = line.IndexOf('F', i);
                s = byte.Parse(line.Substring(i + 1, j - i - 1));

                i = line.IndexOf("F");
                j = line.LastIndexOf('\'');
                f = byte.Parse(line.Substring(i + 1, j - i - 1));
            }
            ...
            return new SecsMessage((byte)s, (byte)f, name, rootItem, replyExpected);
        }
        ...
        private static Item Create(this string format, string smlValue)
        {
            switch (format)
            {
        ...
        public static Item ToItem(this byte[] byteAry)
        {
            int idx = 0;
            ...
            return BufferedDecodeItem(byteAry,ref idx);
        }
    }
}
